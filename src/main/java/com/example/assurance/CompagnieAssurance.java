...
public class CompagnieAssurance {
    ...
    public PoliceAssurance creerPolice(String numeroPolice, TypePolice type, double prime, LocalDate dateDebut, LocalDate dateFin  ,String clientId) {
    ...
    public Sinistre declarerSinistre(String id, String desc, double montant, String policeId){
        PoliceAssurance police = rechercherPolice(policeId).orElseThrow();
    ...
    public Optional<PoliceAssurance> rechercherPolice(String id) { 
        return Optional.ofNullable(polices.get(id));
    }
    
    public void annulerPolice(String id){
        rechercherPolice(id).ifPresent(PoliceAssurance::annuler);
    }
